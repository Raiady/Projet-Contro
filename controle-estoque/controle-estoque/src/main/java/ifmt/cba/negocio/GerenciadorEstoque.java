...
public class GerenciadorEstoque {
    ...
    public void adicionarEstoqueProduto(ProdutoVO produtoVO, int quantidade) 
    throws Exception {
      ...
      public void baixarEstoqueProduto(ProdutoVO produtoVO, int quantidade) 
      throws Exception {
        if (produtoVO != null || quantidade > 0) {
          if (this.listaProduto.indexOf(produtoVO) >= 0) {
            ProdutoVO produtoVOTemp = this.listaProduto.get(this.listaProduto.indexOf(produtoVO));
    ...
    public float totalizarValorEstoqueProduto(ProdutoVO produtoVO) {
        ...
        if (produtoVO != null) {
            valorEstoque = produtoVO.getValorUnitario() * produtoVO.getEstoque();
        }
        return valorEstoque;
    }

    public float totalizarValorEstoqueGeral() {
        float valorEstoqueGeral = 0.0f;
        for (ProdutoVO produtoVO : this.listaProduto) {
            valorEstoqueGeral += totalizarValorEstoqueProduto(produtoVO);
        }
        return valorEstoqueGeral;
    }
}
